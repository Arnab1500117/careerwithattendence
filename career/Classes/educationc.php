...
public function postgraduateInsert($data, $userId){
		...
		$minor = $this->fm->validation($data['minor']);
		$cgpa 		 = $this->fm->validation($data['cgpa']);
		$educationalStatus = $this->fm->validation($data['educationalStatus']);
		$pyearid 	 = $this->fm->validation($data['pyearid']);
		$psemester  		 = $this->fm->validation($data['psemester']);
		$admissionYear		 = $this->fm->validation($data['admissionYear']);
		$asemester 		 = $this->fm->validation($data['asemester']);
		$deptHead  		 = $this->fm->validation($data['deptHead']);
		...
		$minor = mysqli_real_escape_string($this->db->link, $minor);
		$cgpa 		 = mysqli_real_escape_string($this->db->link, $cgpa);
		$educationalStatus = mysqli_real_escape_string($this->db->link, $educationalStatus);
		$pyearid	 = mysqli_real_escape_string($this->db->link, $pyearid);
		$psemester 		 = mysqli_real_escape_string($this->db->link, $psemester);
		$admissionYear		 = mysqli_real_escape_string($this->db->link, $admissionYear);
		$asemester 		 = mysqli_real_escape_string($this->db->link, $asemester);
		$deptHead  		 = mysqli_real_escape_string($this->db->link, $deptHead);
	

		if ($uId == "" || $deid == "" ||  $studydeptId == "" ||  $educationalStatus == "" || $pyearid == "" || 
			$psemester == "" || $admissionYear == "" || $asemester == ""  ) {
			$msg = "Select or Fill All The Data";
			return $msg;
		}
		else{
			$query = "INSERT INTO tbl_postgraduate(userId, uId, deid, studydeptId,minor, cgpa, educationalStatus, pyearid, psemester, admissionYear, asemester, deptHead) VALUES('$userId', '$uId', '$deid','$studydeptId','$minor', '$cgpa', '$educationalStatus', '$pyearid', '$psemester', '$admissionYear', '$asemester', '$deptHead' )";
			$result = $this->db->insert($query);
			if ($result) {
					echo "<script>window.location = 'education.php'</script>";
			}else{
