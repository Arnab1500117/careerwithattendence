...
following link:
https://career.keal.com.bd/LoginRegistrationForm/my_jobs.php
...
https://career.keal.com.bd/LoginRegistrationForm/re_schedule.php
...
01844046677";
	...
	public function getPaddressby($uId){
     ...
     public function getrescheduleapplicantistatus($uId){
          $query = "SELECT * FROM tbl_disappertime WHERE userId = '$uId' LIMIT 1";
          $result = $this->db->select($query);
          return $result;
     }
     public function getApplicant($date){
      ...
      public function getapplicantistatus($uId){
          $query = "SELECT * FROM tbl_interview WHERE userId = '$uId' LIMIT 1";
          $result = $this->db->select($query);
          return $result;
      }
       public function getApplicantreschedule($date){
      	//$date = date('Y-m-d');
      	 $query = "SELECT p.*, r.userName , j.jobtitle FROM tbl_disappertime as p, tbl_user_reg as r, tbl_jobtitle as j WHERE  p.userId = r.regId AND p.jId = j.jId AND p.status = '1' AND ddate = '$date'
      	  			 ORDER BY p.id DESC";
      	  $result = $this->db->select($query);
           return $result;
...
01844046677";
         ...
         public function getcircularidjobtitle(){
			$query = "SELECT p.*, j.jobtitle, d.deptName
				FROM tbl_user_job as p, tbl_jobtitle as j, tbl_department as d
				WHERE p.jId = j.jId AND p.dId=d.dId ORDER BY p.jsId DESC";
	
		$result= $this->db->select($query);
		return $result;
		}
      
         public function getJobCircularId($uId,  $jId){
