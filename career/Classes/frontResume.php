...
<?php
	...
	*/
	...
	public function getpgraduationby($uId){
			$query = "SELECT p.*, c.pyear, a.ayear, d.studyDept, u.uName
				FROM tbl_postgraduate as p, tbl_passingyear as c, tbl_admissionyear as a, tbl_studydept as d, tbl_university as u
				WHERE p.pyearid = c.pyearid AND p.admissionYear = a.ayearid AND p.studydeptId = d.studydeptId AND p.uId = u.uId AND userId = '$uId'";
			$value = $this->db->select($query);
			return $value;
		}
