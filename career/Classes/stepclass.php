...
I Agree:
http://career.keal.com.bd/LoginRegistrationForm/admin/selection.php?phone=$phone 
...
http://career.keal.com.bd/LoginRegistrationForm/admin/reject_selection.php?phone=$phone
...
";
     ...
     public function getallpreselect(){
        ...
        public function getallPselect(){
     	$query = "SELECT p.*, n.userName, j.jobtitle
     	FROM step_07 as p, tbl_user_reg as n, tbl_jobtitle as j WHERE p.userId = n.regId AND p.jId = j.jId AND jobstatus = '1'";
     	$result = $this->db->select($query);
     	return $result;
     }
...
Link: https://career.keal.com.bd/LoginRegistrationForm/reference.php
...
public function selectAttendencePersonlist($uId, $date){
	 $query = "SELECT * FROM tbl_user_reg WHERE regId = '$uId'";
	 $res = $this->db->select($query);
	 if ($res) {
	 	while ($data = $res->fetch_assoc()) {
	...
	$insert_row = "INSERT INTO employee(userId, jId, user, email, phone, sdate) VALUES('$uId', '$jId', '$name', '$email', '$phone', '$date')";
	$inserted = $this->db->insert($insert_row);
	if ($inserted) {
								?>
			<script>var my_time = new Date(); alert('Shift Into Employee Table...'+my_time);
